...
def render_agent_settings_section():
    ...
    st.session_state.web_search_enabled = st.checkbox(
        ...
        help="If enabled, the agent can use web search when its knowledge base is insufficient."
    )
    ...
    if trace_events:
        with st.expander("Agent Workflow Trace"):
            for event in trace_events:
                # Removed icon mapping entirely
                st.subheader(f"Step {event['step']}: {event['node_name']}")
